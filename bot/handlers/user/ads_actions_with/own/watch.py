...
async def edit_ad(call: types.CallbackQuery, state: FSMContext, db: Repository):
    data = await state.get_data()

    await state.set_state(EditAd.param_to_change)
    await state.update_data({
        "ad_for_edit": (await db.get_user_ads(call.from_user.id))[data["current_ad"]],
...
async def delete_ad(call: types.CallbackQuery, state: FSMContext, bot: Bot, db: Repository):
    data = await state.get_data()
    ...
    ad = (await db.get_user_ads(user_id))[data["current_ad"]]

    await db.delete_ad(ad_to_delete=ad)
    await state.clear()
