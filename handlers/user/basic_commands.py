...
async def start(
    ...
    if not await db.user_exists(user_id):
        ...
        await bot.send_message(user_id, 'Добро пожаловать в бота по выставке обьявлений! Для справки можете зайти в /help', reply_markup=mp.main_menu)
...
async def creator(msg: types.Message):
    await msg.answer(msg.from_user.id, 'Cоздатель @akiko233')


@router.message(Command(commands=['start']))
async def on_empty_username(msg: types.Message):
